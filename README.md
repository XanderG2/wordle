...
- `main.py` is the pure python version. You can run either but this version is recommended for just python.
- `main_exe.py` is the version built into an .exe file. It includes a different way of finding the word file, which is useful for the .exe, where it is bundled in.
